...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $skills = array();
    for ($i = 1; $i <= 5; $i++) {
        $skill = sanitize_input($_POST["skill" . $i]);
        if (!empty($skill)) {
            $skills[] = $skill;
        }
    }
    ...
    <div class="content flex">
        <div class="sidebar-div px-1 lg:inline-block w-[20%] bg-slate-100">
            <div class="sidebar h-full w-56 bg-slate-200 lg:block hidden relative z-50">
                ...
                <div class="name-div flex flex-col justify-center mt-2 h-20 border-b-4 border-white">
                    <p class="text-xl font-semibold text-center">Silicon Techlab</p>
                    <p class="font-sans text-center">id:22mmcb24</p>
                </div>
                <div class="dash-page w-56 h-60 bg-slate-200 flex flex-col items-center">
                    <div class="myprofile py-3">
                        <p class="font-serif text-xl"><a href="">My Profile</a></p>
                    </div>
                    <div class="dashboard py-3">
                        <p class="font-serif text-xl"><a href="">Dashboard</a></p>
                    </div>
                    <div class="applyjob py-3">
                        <p class="font-serif text-xl"><a href="">Find Candidate</a></p>
                    </div>
                    <div class="logout py-3">
                        <button class="px-3 py-1 bg-red-600 rounded-md">logout</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="main-content flex flex-col items-center w-full max-h-full bg-slate-200 overflow-x-auto">
            <div class="flex flex-col items-center mt-4">
                ...
                <input type="text" class="p-2 mt-4 border-2 border-gray-300 rounded-md w-64" disabled>
                <input type="text" class="p-2 mt-4 border-2 border-gray-300 rounded-md w-64" disabled>
            ...
            <form id="skill-modal" class="modal">
                <div class="modal-content">
                    <span class="close">&times;</span>
                    <h2 class="text-xl font-bold mb-4">Add Skills</h2>
                    <div class="flex flex-col mb-4">
                        <label for="skill1" class="mb-2">Skill 1:</label>
                        <input type="text" id="skill1" name="skill1" class="p-2 border-2 border-gray-300  rounded-md" required>
                    </div>
                    <div class="flex flex-col mb-4">
                        <label for="skill2" class="mb-2">Skill 2:</label>
                        <input type="text" id="skill2" name="skill2" class="p-2 border-2 border-gray-300 rounded-md" required>
                    </div>
                    <div class="flex flex-col mb-4">
                        <label for="skill3" class="mb-2">Skill 3:</label>
                        <input type="text" id="skill3" name="skill3" class="p-2 border-2 border-gray-300 rounded-md" required>
                    </div>
                    <div class="flex flex-col mb-4">
                        <label for="skill4" class="mb-2">Skill 4:</label>
                        <input type="text" id="skill4" name="skill4" class="p-2 border-2 border-gray-300 rounded-md" required>
                    </div>
                    <div class="flex flex-col mb-4">
                        <label for="skill5" class="mb-2">Skill 5:</label>
                        <input type="text" id="skill5" name="skill5" class="p-2 border-2 border-gray-300 rounded-md" required>
                    </div>
                    <button type="submit" id="submit-skill-btn" name="add" class="px-4 py-2 bg-green-600 text-white rounded-md">Add Skills</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function addSkill(event) {
            ...
            const existingSkills = Array.from(skillsList.querySelectorAll('span')).map(skillElement => skillElement.textContent.toLowerCase());
            const newSkills = [];

            skillInputs.forEach(function (input) {
                const skill = input.value.trim();
                if (skill && !existingSkills.includes(skill.toLowerCase())) {
